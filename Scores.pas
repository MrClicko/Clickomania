...
type
  ...
  public
    procedure Update(G: TGameType; NewEntry, NewBonus: Integer);
...
procedure TfrmScores.Update(G: TGameType; NewEntry, NewBonus: Integer);
var SE : TScoreEntry;
    i  : Integer;
    F  : Double;
    Item : TListItem;
...
for i := 0 to NrOfScores do begin
   ...
   listScores.Font.Size := 7;
   if SE.Pt > 0 then begin
      Item := listScores.Items.Add;
      Item.Data := Pointer(i);
      if i < 10
         then Item.Caption := IntToStr(i+1)
         else Item.Caption := '*';
...
if not IsBig then begin
   ...
   panelList.Left := 4;
   panelList.Top := 43;
   panelList.Height := 200;
   if panelWebscore.Visible
      then frmScores.Height := 275 + panelWebscore.Height
      else frmScores.Height := 275;
...
if (Bonus > 0) then BonusStr := '&'+WriteBonus+'='+ IntToStr(Bonus)
               else BonusStr := '';
...
try
   ...
   {$IFDEF AlternativeScoreWrite}
   AssignFile(F, 'e:\temp\Ausgabe.html');
   Rewrite(F);
   Writeln(F, ServerAnswer);
   CloseFile(F);
   {$ENDIF}
