...
uses
  ...
  FileCtrl, ImgList, jpeg, ImgRoutines, PicPanel, Menus, StoneStat,
  hh, LangSupp, DdeMan;
...
type
  TfrmMain = class(TForm)
    ...
    procedure PaintDarkBack(Sender: TObject);
    procedure PaintBack(Sender: TObject);
  ...
  private
    ...
    procedure TileBMP(Dest: TPicPanel; Bmp: TBitmap);
...
procedure TfrmMain.TileBMP(Dest: TPicPanel; Bmp: TBitmap);
var x, y  : Integer;
    xx, yy: Integer;
    DRect : TRect;
begin
x := 0; y := 0;
while (x < (Dest.Left + Dest.Width)) do begin
   while (y < (Dest.Top + Dest.Height)) do begin
      xx := x - Dest.Left;
      yy := y - Dest.Top;
      DRect := Rect(xx, yy, xx+bmp.Width, yy+bmp.Height);
      Dest.Canvas.CopyRect(DRect, Bmp.Canvas, Bmp.Canvas.ClipRect);
      inc(y, Bmp.Height);
      end;
   inc(x, Bmp.width);
   y := 0;
...
procedure TfrmMain.PaintDarkBack(Sender: TObject);
begin
TileBMP(TPicPanel(Sender), BacktileDark);
end;

procedure TfrmMain.PaintBack(Sender: TObject);
begin
TileBMP(TPicPanel(Sender), Backtile);
end;
...
BackTileDark := TBitmap.Create;
LoadJPEGfromRes(TBID+IntToStr(BTIndex), Backtile);
LoadJPEGfromRes(TBID+IntToStr(BTIndex), BacktileDark);
ImgRoutines.Helligkeit(BacktileDark, Backtile,-85);
end;
